...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
    ...
    for (int k = 0; k < wtx.vout.size(); ++k)
    {
        ...
        if (wtx.IsCoinBase())
            strHTML += "<br>" + tr("Generated coins must mature 22 blocks before they can be spent. When you generated this block, it was broadcast to the network to be added to the block chain. If it fails to get into the chain, its state will change to \"not accepted\" and it won't be spendable. This may occasionally happen if another node generates a block within a few seconds of yours.") + "<br>";
